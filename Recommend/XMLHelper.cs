...
namespace Recommend
{
    public class XMLHelper
    {
        ...
        public List<Follower> SelectAllFollower()
        {
            ...
            foreach (XmlNode item in rootNode.ChildNodes)
            {
                ...
                follower.Id = index;
                follower.NameUrl = item.Attributes["Url"].Value.ToString();
                string fUrl = item.Attributes["FollowerStr"].Value.ToString();
                string count = item.Attributes["Count"].Value.ToString();
                if (String.IsNullOrEmpty(fUrl) || Convert.ToInt32(count) <= 0)
                {
                    follower.FollowerUrl = new List<string>();
                    follower.Count = 0;
                }
                else
                {
                    if (fUrl.Contains(","))
                    {
                        string[] fers = fUrl.Split(',');
                        List<string> followerList = new List<string>();
                        foreach (var fer in fers)
                        {
                            if (!String.IsNullOrEmpty(fer))
                                followerList.Add(fer);
                        }
                        follower.FollowerUrl = followerList;
                        follower.Count = Convert.ToInt32(count);
                    }
                    else
                    {
                        follower.FollowerUrl = new List<string>();
